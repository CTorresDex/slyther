---
name: update-command
description: Updates an existing command artifact's source file to satisfy a natural-language change request, given its command path, erroring if the command doesn't exist.
---
...
2. **deterministic** — Run .gstudio/artifacts/command/scripts/update/step-2.sh <command-path> <change-request>, where <command-path> is the slash-separated command path (e.g. create/artifact) and <change-request> is ignored by this step.
