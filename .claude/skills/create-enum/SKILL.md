---
name: create-enum
description: Scaffolds a new enum file at src/src/enums/{Name}.enum.ts with minimal members, given an id and optional member list.
---
...
2. **deterministic** — Run .gstudio/artifacts/enum/scripts/create/step-2.ts <id> where <id> is the enum identifier (e.g. `.gstudio/artifacts/enum/scripts/create/step-2.ts paymentStatus`).
