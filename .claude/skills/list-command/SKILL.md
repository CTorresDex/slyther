---
name: list-command
description: "Lists command artifacts in the codebase matching an optional search term, printing each as [command path]: [file path]"
---
...
2. **deterministic** — Run .gstudio/artifacts/command/scripts/list/step-2.sh [search-term] to list every command file under src/src/commands, optionally restricted to those whose content contains search-term, one per line as `[command path]: [file path]`.
