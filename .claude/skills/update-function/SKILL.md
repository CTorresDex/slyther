...
description: Updates an existing function artifact's source file to satisfy a natural-language change request, given its id, erroring if the function doesn't exist.
---
...
2. **deterministic** — Run .gstudio/artifacts/function/scripts/update/step-2.ts <id> <change-request> to verify the function identified by `<id>` exists before continuing the update action.
