---
name: delete-command
description: Deletes the command artifact with the given command path, removing its file and cleaning up dangling references to it across the codebase.
---
...
3. **deterministic** — Run .gstudio/artifacts/command/scripts/delete/step-3.sh <path>, where <path> is the command's slash-separated path (e.g. create/artifact) written exactly as it appears in its file location.
