...
description: Scaffolds a new singleton file at src/singletons/{name}.singleton.ts given an id and optional initial instance content, failing if the file already exists.
---
...
1. **llm** — Input: {id} and optionally the instance the singleton holds

2. **deterministic** — Run .gstudio/artifacts/singleton/scripts/create/step-2.ts <id> to scaffold the singleton, where <id> is the singleton's identifier (any case, will be converted to camelCase).

3. **llm** — The singleton should contain only the minimum amount of code to fulfill the requirements, following YAGNI principle.
