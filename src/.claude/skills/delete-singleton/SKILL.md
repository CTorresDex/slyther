...
description: Removes a singleton file by id, then finds and removes all dangling references to it in the codebase.
---
...
2. **deterministic** — Run `.gstudio/artifacts/singleton/scripts/delete/step-2.ts <id>` where `<id>` is the singleton's id.

3. **deterministic** — Run .gstudio/artifacts/singleton/scripts/delete/step-3.sh <id> to print every `file path:line number` reference to the singleton identified by `<id>` found in the codebase.
