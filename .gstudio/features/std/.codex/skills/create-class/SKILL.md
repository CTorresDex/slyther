---
name: create-class
description: Scaffolds a new class file at src/classes/{Name}.class.ts given a class id and optional initial content, failing if the file already exists.
---
...
2. **deterministic** — Run .gstudio/artifacts/class/scripts/create/step-2.sh <id> [content] where <id> is the class identifier (converted to PascalCase for the file and class name) and the optional [content] is the body to place inside the class.
...
2. **deterministic** — Run `.gstudio/artifacts/class/scripts/evaluate/step-2.sh <id>` where `<id>` is the class id to evaluate.
