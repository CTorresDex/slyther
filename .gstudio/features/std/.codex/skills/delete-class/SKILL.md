---
name: delete-class
description: Deletes a class file at src/classes/{name}.class.ts by id, then finds and removes all dangling references to it in the codebase.
---
...
2. **deterministic** — Run .gstudio/artifacts/class/scripts/delete/step-2.sh <id> with the class id as the sole argument.

3. **deterministic** — Run .gstudio/artifacts/class/scripts/delete/step-3.sh <id> from the project root, where `<id>` is the class identifier (the same id used to create/delete it).
