...
description: Modifies an existing singleton file at src/singletons/{name}.singleton.ts to satisfy a given change request, erroring if the singleton does not exist.
---
...
2. **deterministic** — Run .gstudio/artifacts/singleton/scripts/update/step-2.ts <id> <change-request> to verify the singleton for <id> exists before applying the change request.
