---
name: create-command
description: Scaffolds the command file at src/src/commands/create/artifact.command.ts given a command path and optional content, exiting 1 if it already exists.
---
...
2. **deterministic** — Run .gstudio/artifacts/command/scripts/create/step-2.sh <path> [content], where <path> is the slash-separated command path (e.g. create/artifact) and the optional [content] is the full file content to use instead of the default scaffold.
